...
### BUG-39: 摄像头插拔导致画面卡死且UI状态误判为“在线”的故障修复
*   **测试场景**：拔出 USB 摄像头后，界面上的实时监控画面定格卡死，但摄像头指示灯依然显示为绿色的“在线”，并未触发系统掉线警示。
*   **排查思路与根本原因**：
    1. 观察 UI 的在线状态判定代码，原本使用的是：
       ```cpp
       if (videoLabel && videoLabel->pixmap() && !videoLabel->pixmap()->isNull() && current_source_mode.load() == 0)
       ```
    2. 当 USB 摄像头热拔出时，底层 V4L2 解码线程读不到新帧，停止向无锁队列 `cap_queue` 推送。
    3. UI 的 `updateFrame` 槽函数因而停止被调用，此时 `videoLabel` 上显示的内容为最后一帧成功渲染的缓存画面。
    4. 由于 `videoLabel->pixmap()` 仍保存有这帧画面而非空（Not Null），刷新定时器在半秒一次的扫描中认为画面有效，从而一直显示“在线”绿字。
*   **解决方案**：
    1. **心跳时间戳判定**：在 UI 线程的 `updateFrame` 刷新函数中，新增全局时间戳 `g_last_frame_time_ms`，每次成功渲染一帧，就记录一次系统毫秒级时钟。
    2. **超时离线判定**：在定时状态刷新器中，一旦检测到当前时间与 `g_last_frame_time_ms` 间隔超过 **1.5 秒**，则说明视频流拉取中断。
    3. **自愈交互**：此时立刻主动清理 `videoLabel` 并显示红色虚线边框 `🚨 摄像头已断开，正在尝试重新连接...`，同时置为“离线”状态。
*   **实测验证**：
    1. 主控 Qt 重新编译部署运行。
    2. 拔下 USB 摄像头，1.5 秒内界面刷新，画面定格消失并转为高科技感红框警告，摄像头指示灯显示红字“离线”。
    3. 插回摄像头，画面在一秒内立刻恢复流畅刷新，指示灯自动同步跳转为“在线”。

### BUG-40: 正常上报的物理传感器信号高频刷新导致UI今日安全评分被强行覆盖重设为84分
*   **测试场景**：当系统中存在 AI 违规（如未戴安全帽）时，分数应扣减，但分数每半秒就会闪跳回 84 分，无法保持正确的安全评分。
*   **排查思路与根本原因**：
    1. 从核每 500ms 通过 RPMsg 向上报送传感器电平状态，主控端收到数据包后无防抖、无过滤地通过信号槽抛向 UI。
    2. 接收槽函数在处理“正常（0）”时，原先直接硬编码调用了 `scoreWidget->setScore(84, "良好 - 压线40分")`。
    3. 导致每隔 500ms 物理传感器传回正常电平时，就会把 UI 今日安全得分强制还原为 84 分，直接重写并抹去了由 AI 视觉判断得出的扣分记录。
*   **解决方案**：
    1. **RPMsg 去重与防抖状态机**：在 `rpmsg_node.cpp` 中引入 `last_emitted_fire_status` 和 `last_emitted_gas_status` 状态存储。只有在接收的电平状态发生**真实切换**时（如 正常 ➡️ 报警，或 断开 ➡️ 正常），才触发一次 UI 信号通知，屏蔽高频的冗余 0 包发送。
    2. **评分动态还原**：在 UI 的正常状态处理分支中，将硬编码 `84` 改为调用系统的违规计分模型进行**动态计算**：
       ```cpp
       ...
       ```
*   **实测验证**：
    1. 部署运行后，在没有火灾和气体异常时，今日评分由 AI 违规行为独占控制，完美依据检测目标动态滑移（如未戴安全帽实时呈现 95 分），不再闪跳和被强制重置为 84 分。
