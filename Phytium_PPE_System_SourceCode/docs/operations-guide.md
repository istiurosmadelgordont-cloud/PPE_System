...
2. **异构物理联动时延测试 (TC-06)**：
   * **测量原理**：使用示波器或逻辑分析仪精确测量主从核联动时延。
     * **通道 1 (CH1)**：连接主核测试发信起点引脚 `GPIO4_13`（排针 Pin 37）。主核准备发送违规报警的瞬间拉高该引脚。
     * **通道 2 (CH2)**：连接从核控制蜂鸣器物理阻断引脚 `GPIO3_1`（排针 Pin 11）。从核收到报警瞬间拉高该引脚。
     * **测量指标**：单次触发抓取波形，测量 CH1 与 CH2 上升沿之间的时间差 \(\Delta t\)。
   * **实测结果**：**Pass**。经示波器精密测量，跨核联动平均物理时延仅为 **42.84 微秒 (μs)**，远优于 1ms 阈值要求。详见 [06_主从核联动延迟测试报告](file:///d:/飞腾派/CICC1004607+初赛+技术数据(代码类)/ppe4-28/Phytium_PPE_System_SourceCode/docs/06_主从核联动延迟测试报告.md)。
